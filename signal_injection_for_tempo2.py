...
def inject_population_into_psrs(psrs, population, pure_signal=True, add=False, verbose=False):
    ...
    TEMPO2 Note: Modifies pulsars IN-PLACE since they can't be copied.
    """
    for psr in psrs:
...
def inject_population_subset_cached(psrs, population, N_binaries, 
    ...
    TEMPO2 Note: Modifies pulsars IN-PLACE since they can't be copied.
    If you need originals, save residuals before calling:
        original_residuals = {psr.name: psr.residuals.copy() for psr in psrs}
    ...
    # For TEMPO2: We can't copy pulsars, so we modify in-place
    # Enterprise PTA needs the actual pulsar objects, not wrappers
    for psr in psrs:
        psr_name = psr.name
    ...
    # Return the modified pulsars (same objects, modified residuals)
    return psrs