...
def test_sky_CGW_SNR_location(psrs_clean, raw_noise_params, parsed_noise_params, Tspan):
    ...
    # print out the ranked binaries with sky locations and SNRs
    print("\nRanked CGW candidates by optimal SNR:")
    for i, (binary, snr) in enumerate(sorted(zip(binary_rows, cgw_snrs_optimal), key=lambda x: x[1], reverse=True)):
